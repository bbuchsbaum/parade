...
`parade-registry/script-<hash>/logs/`, guessing whether a failure was
OOM or timeout, and having no unified view across the run.

**parade** now provides a cohesive failure diagnosis layer:

- **[`wtf()`](https://bbuchsbaum.github.io/parade/reference/wtf.md)** –
  a single “what went wrong?” entry point that classifies errors,
  suggests fixes, and locates logs
- **Error classification** – automatic categorization into OOM, timeout,
  R error, crash, infrastructure, dependency, and validation failures
...
- **[`run_ls()`](https://bbuchsbaum.github.io/parade/reference/run_ls.md)
  ...
  – pipeline run registry and discovery
- **[`pipeline_top()`](https://bbuchsbaum.github.io/parade/reference/pipeline_top.md)**
  – enhanced live TUI with event feed and classified errors
- **[`failure_patterns()`](https://bbuchsbaum.github.io/parade/reference/failure_patterns.md)**
  – cross-run error comparison to spot persistent vs. flaky failures

## Quick start: diagnosing a failed pipeline

The most common scenario: you submitted a pipeline, some chunks failed,
...
[`wtf()`](https://bbuchsbaum.github.io/parade/reference/wtf.md) does the
heavy lifting: it queries SLURM accounting data (`sacct`), reads `.diag`
...
[`wtf()`](https://bbuchsbaum.github.io/parade/reference/wtf.md) is an S3
generic with methods for every parade result type. All methods return a
`parade_failure_report` object (invisibly) and print a formatted report.
...
This inspects the `.diag` and `.ok` columns to find and classify
failures.
...
By default, only logs from failed chunks are returned. Set
...
The `context` parameter (default 3) controls how many surrounding lines
to show around each match.

## Failure timeline

[`failure_timeline()`](https://bbuchsbaum.github.io/parade/reference/failure_timeline.md)
builds a chronological view of events during a pipeline run. It combines
data from three sources: the event store (JSONL), `.diag` timestamps
...
The offset column (`+H:MM:SS`) shows time since the pipeline was
submitted. This makes it easy to see failure clustering – for instance,
OOM kills arriving together might indicate a shared node running out of
memory.
...
file at `artifacts://runs/{run_id}/events.jsonl`. The event store
captures:

- `run_started`, `run_completed`, `run_failed` – pipeline lifecycle
- `chunk_started`, `chunk_completed`, `chunk_failed` – chunk execution
- `stage_failed`, `stage_retried`, `retry_exhausted` – stage-level
  detail

Events are written via
[`.event_emit()`](https://bbuchsbaum.github.io/parade/reference/dot-event_emit.md),
which is wrapped in
[`tryCatch()`](https://rdrr.io/r/base/conditions.html) so it **never**
blocks or fails the pipeline. Monitoring is purely advisory.
...
Events are consumed internally by
[`failure_timeline()`](https://bbuchsbaum.github.io/parade/reference/failure_timeline.md),
[`pipeline_top()`](https://bbuchsbaum.github.io/parade/reference/pipeline_top.md),
and
[`run_info()`](https://bbuchsbaum.github.io/parade/reference/run_info.md).
...
The event log is human-readable. Each line is a self-contained JSON
object:
...
The JSONL format is NFS-safe (POSIX atomic appends for lines \< 4096
bytes), requires no database dependency, and can be inspected with
standard Unix tools:
...
call registers the run automatically.
...
[`pipeline_top()`](https://bbuchsbaum.github.io/parade/reference/pipeline_top.md)
is an enhanced live monitor that combines the progress tracking of
[`deferred_top()`](https://bbuchsbaum.github.io/parade/reference/deferred_top.md)
with the event feed and classified error summaries.
...
The state column for failed chunks now reads `FAILED:OOM`,
`TIMEOUT:TIMEOUT`, or `FAILED:ERROR` instead of just `FAILED`, giving
you immediate visibility into what kind of failure occurred without
needing to run
[`wtf()`](https://bbuchsbaum.github.io/parade/reference/wtf.md).

## Cross-run comparison with failure_patterns()

When you’re iterating on a pipeline – fixing bugs, adjusting resources,
resubmitting – you want to know which errors are persistent, which are
new, and which were resolved.
[`failure_patterns()`](https://bbuchsbaum.github.io/parade/reference/failure_patterns.md)
...
same logical error gets the same fingerprint even when runtime details
change.
...
- [`vignette("parade-scripts-monitoring")`](https://bbuchsbaum.github.io/parade/articles/parade-scripts-monitoring.md)
  – monitoring individual SLURM script jobs with
...
- [`vignette("parade-slurm-distribution")`](https://bbuchsbaum.github.io/parade/articles/parade-slurm-distribution.md)
  – configuring SLURM resources, chunking strategies, and
  [`dist_slurm()`](https://bbuchsbaum.github.io/parade/reference/dist_slurm.md)
  options
- [`?wtf`](https://bbuchsbaum.github.io/parade/reference/wtf.md) – full
  parameter reference for the failure report
- [`?deferred_errors`](https://bbuchsbaum.github.io/parade/reference/deferred_errors.md)
  – the underlying error aggregation that
  [`wtf()`](https://bbuchsbaum.github.io/parade/reference/wtf.md) builds
  on
