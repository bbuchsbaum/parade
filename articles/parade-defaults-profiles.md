...
## User-managed site profiles

Cluster policy and hardware change independently of parade releases, so
parade does not hard-code profiles for named sites. Keep those values in
your user or project config instead. You can define one with
[`slurm_defaults_set()`](https://bbuchsbaum.github.io/parade/reference/slurm_defaults_set.md),
or register a list or fluent profile and ask parade to persist it:

``` r

profile_register(
  "my_cluster_whole_node",
  list(
    partition = "compute",
    time = "4h",
    cpus_per_task = 64,
...
# A string profile works directly in mapping APIs.
jobs <- slurm_map(
  inputs,
  analyze_one,
  .resources = "my_cluster_whole_node",
  .packed = TRUE,
  .workers_per_node = 64
)
```

`whole_node` and `cores_per_node` are local safety metadata, not
scheduler directives. They let
[`slurm_map()`](https://bbuchsbaum.github.io/parade/reference/slurm_map.md)
warn if standard mode would fan a multi-item map out into separate
whole-node jobs. The warning is conservative and does not change
submission behavior; choose `.packed = TRUE` when packing is
appropriate. Use
`profile_remove("my_cluster_whole_node", persist = TRUE)` to remove a
persisted profile.
...
1.  The current R session (`parade.slurm.defaults` for the default
    profile and `parade.slurm.profiles` for named profiles) so
    subsequent
    [`slurm_resources()`](https://bbuchsbaum.github.io/parade/reference/slurm_resources.md)
    calls see changes immediately.
...
| `profile_register("name", list(...), persist = TRUE)` | Persist a user/site profile | `profile_register("lab", list(time = "2h"), persist = TRUE)` |
| `profile_remove("name", persist = TRUE)` | Remove a persisted profile | `profile_remove("lab", persist = TRUE)` |
