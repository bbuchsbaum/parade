...
allocation + dispatcher) but you can get close today using two ideas:
...
- **Packed jobs** for
  ...
  utilize a node.
- **Target job count** for
  [`submit()`](https://bbuchsbaum.github.io/parade/reference/submit.md)/[`dist_slurm()`](https://bbuchsbaum.github.io/parade/reference/dist_slurm.md):
  pick a fixed number of jobs to keep in flight, independent of the
  number of groups in your grid.
...
For flows, use `target_jobs` to control chunking at submission time:
...
fl <- fl |>
  distribute(dist_slurm(
    ...
    within = "multicore",
    workers_within = 196,
    resources = list(nodes = 1, ntasks = 1, cpus_per_task = 196, time = "2h", mem = "64G"),
    target_jobs = 20               # ~10 nodes * oversubscribe 2
  ))
```

**Important limits:** this is still *static partitioning* (chunks are
fixed at submit time). If task durations are highly variable, a true
pool/dispatcher (future work) will reduce tail latency further.
