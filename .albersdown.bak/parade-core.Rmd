...
vignette: |
  %\VignetteIndexEntry{2. Core Concepts: flows, types, execution} %\VignetteEngine{knitr::rmarkdown} %\VignetteEncoding{UTF-8}
params:
  family: red
css: albers.css
resource_files:
...
So far every stage has been an inline `function(...)`. That works great for
short lambdas, but as logic grows past a few lines the pipe becomes hard to
read. Parade offers three stage styles — pick the one that fits your code:

| Style | Best for | How it looks |
|-------|----------|-------------|
| `stage()` | Short lambdas (1-5 lines) | `stage("id", function(x) list(y = x^2), ...)` |
| `code_stage()` | Medium blocks (5-50 lines) | Code block lives inline, no `function()` wrapper |
| `stage_def()` + `add_stage()` | Reusable / long stages | Define once, add to any flow |

### `stage()` — inline lambdas

You already know this one. It reads beautifully when the function is short:
...
`function(...)` clutters the pipeline. `code_stage()` captures a bare code
block instead. Grid columns and upstream outputs are available as plain
variables — no function signature to maintain:
...
`stage()` — `skip_when`, `sink`, `prefix`, `resources`, retries — works
with `code_stage()` too.

**Tip**: `code_stage()` captures the block via `substitute()`, so variables
...
separately with `stage_def()` and plug them in with `add_stage()`:
...
`stage_def()` stores the function, dependencies, schema, and any extra
arguments (constants, `skip_when`, `sink`, etc.). `add_stage()` unpacks
them into a regular `stage()` call.
