...
vignette: |
  %\VignetteIndexEntry{1. Unified API: functions and scripts} %\VignetteEngine{knitr::rmarkdown} %\VignetteEncoding{UTF-8}
params:
  family: red
css: albers.css
resource_files:
...
- Overview: high-level orientation — [Overview](articles/parade-overview.html)
- Quickstart: your first pipeline — [Quickstart](articles/parade-quickstart.html)
- Sinks & Artifacts: robust I/O — [Sinks](articles/parade-sinks.html)
...
When you have many tasks, HPC admins often prefer *fewer jobs that fully use a node*.
Use packed mode:
...
If you want to think in terms of a cluster shape (e.g., 10 nodes × 196 cores),
use `slurm_map_cluster()`:
...
If you’re using `flow()` + `submit()`, you can express “10 nodes × 196 cores” as a
distribution spec and keep the DAG itself unchanged:
...
This is still static partitioning at submit time. If task durations vary widely, consider
dispatcher-style backends (`dist_mirai(..., dispatcher = TRUE)` or `dist_crew(...)`) for
...
- resources / .resources: how much you ask SLURM for
  - Accepts a profile name (e.g., "gpu", "standard"), a chained profile object (profile() %>% res_time("2h") %>% cpus(8)), or a simple list.
