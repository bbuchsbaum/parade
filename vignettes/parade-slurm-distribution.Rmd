...
dispatcher) but you can get close today using two ideas:

- **Packed jobs** for `slurm_map()`: run many tasks per SLURM job and use within‑node
  parallelism to fully utilize a node.
- **Target job count** for `submit()`/`dist_slurm()`: pick a fixed number of jobs to
  keep in flight, independent of the number of groups in your grid.
...
**Important limits:** this is still *static partitioning* (chunks are fixed at submit time).
If task durations are highly variable, a true pool/dispatcher (future work) will reduce
tail latency further.
