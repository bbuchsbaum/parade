...
vignette: >
  %\VignetteIndexEntry{5. SLURM Distribution: cluster workflows and HPC}
...
# Local development: test on a subset
fl_local <- fl |> 
...
fl <- flow(grid) |> stage("analyze", expensive_computation)
...
fl <- flow(grid) |> stage("process_trial", trial_analysis)
...
fl_dev <- flow(grid_dev) |> distribute(dist_local(workers_within = 2))

# Production: full dataset on cluster  
grid_prod <- param_grid(subject = paste0("sub", 1:100), session = 1:4)
fl_prod <- flow(grid_prod) |> distribute(dist_slurm(
  by = "subject",
  workers_within = 4,
  chunks_per_job = 1,
