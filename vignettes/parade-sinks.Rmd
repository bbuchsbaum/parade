...
When using formulas with `sink_quick()`:
- `.x` - the object to write
- `.path` - the full target path (with extension)
- `.base` - the target path without extension
...
Files are written to `tempdir()/parade-quick-TIMESTAMP/` and cleaned up automatically by R.
