...
complex_flow <- flow(grid) |>
  ...
  stage(id = "load", 
        f = function(participant, condition) {
          # Load raw data
          # In practice, replace with your actual data loading function
          # e.g., read.csv(resolve_path(paste0("data://", participant, "_", condition, ".csv")))
          list(raw_data = read_data(participant, condition))  # user-defined function
        },
  ...
  stage(id = "clean",
        ...
        f = function(load.raw_data) {
          # Clean the data
          # Replace with your data cleaning logic
          list(clean_data = remove_outliers(load.raw_data))  # user-defined function
        },
  ...
  stage(id = "validate",
        ...
        f = function(model.model) {
          # Cross-validation
          # Replace with your validation logic, e.g., using caret or mlr3
          list(cv_score = cross_validate(model.model))  # user-defined function
        },
  ...
  stage(id = "report",
        ...
        f = function(model.model, validate.cv_score) {
          ...
          list(summary = create_report(model.model, validate.cv_score))  # user-defined function
        },
...
model_flow <- flow(grid) |>
  stage(
    ...
    f = function(participant) {
      # Fit a complex model
      # Replace get_data() with your data loading logic
      model <- lm(y ~ x1 + x2 + x3, data = get_data(participant))  # user-defined function
