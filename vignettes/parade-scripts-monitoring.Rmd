...
vignette: >
  %\VignetteIndexEntry{8. SLURM Scripts: submission and monitoring}
...
> Note: Code evaluation is disabled to keep builds fast and avoid interacting with live cluster resources during checks. Copy code into an interactive session with SLURM access to run.
...
# Or submit a function directly (new in 0.12.0)
job <- slurm_call(
  function(fold) {
...
You can submit a single function call and still use the full jobset verb surface by
wrapping the result as a jobset. Use the `.as_jobset = TRUE` flag on `slurm_call()`
to opt in to this style:
...
# Same verbs you use for slurm_map() work here too
...
# After job completes, load the result
script_await(job)
