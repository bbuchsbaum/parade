...
vignette: >
  %\VignetteIndexEntry{9. SLURM Profiles: Managing Multiple Job Types}
...
> Note: Code evaluation is disabled to keep builds fast and cluster-agnostic. Use these snippets in an interactive session configured for your site.
...
- **Defaults**: When you use `slurm_defaults_set()` without a `profile` parameter, you're setting the "default" profile
- **Profiles**: Named sets of SLURM parameters that you can switch between

In reality, "defaults" and "profiles" use the same underlying mechanism - defaults are just the profile named "default".
...
Cluster policy and hardware change independently of parade releases, so parade
does not hard-code profiles for named sites. Keep those values in your user or
project config instead. You can define one with `slurm_defaults_set()`, or
register a list or fluent profile and ask parade to persist it:
...
`whole_node` and `cores_per_node` are local safety metadata, not scheduler
directives. They let `slurm_map()` warn if standard mode would fan a multi-item
map out into separate whole-node jobs. The warning is conservative and does not
change submission behavior; choose `.packed = TRUE` when packing is appropriate.
Use `profile_remove("my_cluster_whole_node", persist = TRUE)` to remove a
persisted profile.
...
1. The current R session (`parade.slurm.defaults` for the default profile and
   `parade.slurm.profiles` for named profiles) so subsequent
   `slurm_resources()` calls see changes immediately.
2. (Optionally) The on-disk config stored at `file.path(paths_get()$config, "parade.json")` when `persist = TRUE`. This makes the profile available to future sessions and collaborators who share the project.
...
`slurm_resources(profile = "...")` merges the stored defaults with any inline overrides you pass, then normalizes the result via `batchtools::batch_resources()`. The final list travels with each `submit_slurm()`/`dist_slurm()` call into batchtools’ registry, where it ultimately feeds the SLURM template.

### Relationship to `slurm_template_set()`

`slurm_template_set()` merely remembers which batchtools template file to use (also inside the same JSON config). Templates stay completely decoupled from profiles: profiles define what resources you request, while the template dictates *how* those resources are rendered into `#SBATCH` lines. Keeping them separate means you can swap templates (e.g., different clusters or accounting directives) without touching your resource presets, and vice versa.
