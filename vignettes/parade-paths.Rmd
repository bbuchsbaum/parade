...
vignette: >
  %\VignetteIndexEntry{2. Smart Path Management: Write Once, Run Anywhere}
...
model <- readRDS("artifacts://models/brain_model.rds")
results <- analyze_brain(model)
saveRDS(results, "artifacts://outputs/results.rds")
```
...
saveRDS(model, "artifacts://model.rds")
```
...
# On your laptop
paths_init()
saveRDS(big_model, "artifacts://models/final.rds")
...
brain_atlas <- readRDS("data://references/MNI_atlas.rds")
subjects <- read.csv("data://participants.csv")
```

**`artifacts://`** - Analysis outputs (large files)
```r
# Models, results, processed data - goes to fast scratch storage
saveRDS(fitted_model, "artifacts://models/model_v2.rds")
write.csv(results, "artifacts://results/final_results.csv")
```
...
analyze_subjects <- function(subjects) {
  for (subj in subjects) {
    data <- readRDS(sprintf("data://subjects/%s.rds", subj))
    results <- process_subject(data)
    saveRDS(results, sprintf("artifacts://results/%s.rds", subj))
  }
}
```
...
|-------|---------|---------|
| `data://` | Input data | `read.csv("data://input.csv")` |
| `artifacts://` | Outputs | `saveRDS(m, "artifacts://model.rds")` |
